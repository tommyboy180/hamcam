 ...
 *
 * Rewrites config.php and go2rtc.yaml in place (they ship with working
 * example values, so there's nothing to "create" — just overwrite),
 * and keeps .env in sync for docker-compose.
 */
...
// ---------------------------------------------------------------- templates --
// These are rendered fresh every time the form is saved — config.php and
// go2rtc.yaml are fully regenerated from here, not patched in place. Every
// field is a __TOKEN__, including the numeric/boolean ones; this string is
// just text being substituted, never executed directly, so there's no PHP
// syntax constraint on it the way there is on the files it produces.

const CONFIG_TEMPLATE = <<<'PHPTPL'
<?php
// ============================================================
//  HamCAM Configuration — generated by the setup wizard (setup.php)
//  Don't edit by hand; re-run the wizard instead.
// ============================================================

define('HAMCAM_SETUP_COMPLETE', true);

// --- Authentication ---
define('HAMCAM_PASSWORD_HASH', '__HAMCAM_PASSWORD_HASH__');  // bcrypt
define('SESSION_TIMEOUT', 3600);                              // seconds (1 hour)

// --- Camera Settings ---
define('CAMERA_IP',        '__CAMERA_IP__');
define('CAMERA_RTSP_PORT', __CAMERA_RTSP_PORT__);
define('CAMERA_USER',      '__CAMERA_USER__');
define('CAMERA_PASS',      '__CAMERA_PASS__');

define('RTSP_STREAM_HD', '__RTSP_STREAM_HD__');
define('RTSP_STREAM_SD', '__RTSP_STREAM_SD__');
...
define('ONVIF_PORT', __ONVIF_PORT__);

// --- Motion Detection ---
define('MOTION_ACTIVE_START', '__MOTION_ACTIVE_START__');
define('MOTION_ACTIVE_END',   '__MOTION_ACTIVE_END__');
define('MOTION_ALWAYS_ON',    __MOTION_ALWAYS_ON__);
define('MOTION_THRESHOLD',    __MOTION_THRESHOLD__);
define('MOTION_COOLDOWN',     __MOTION_COOLDOWN__);
...
const GO2RTC_TEMPLATE = <<<'YAMLTPL'
# Generated by the setup wizard (setup.php). Don't edit by hand;
# re-run the wizard instead.
...
// Plain string token substitution — no regex, no backreference quirks
// (matters because bcrypt hashes contain literal '$' characters).
...
// go2rtc only reads go2rtc.yaml at process startup — it won't notice the
// file we just rewrote on its own. It exposes POST /api/restart specifically
// for this: an in-process restart that re-reads its config from disk. Since
// hamcam and go2rtc share a Docker network, we can call this directly —
// no Docker socket access needed, unlike restarting the *container* would.
function reload_go2rtc(): bool {
    $ctx = stream_context_create([
        'http' => ['method' => 'POST', 'timeout' => 3, 'ignore_errors' => true],
    ]);
    $result = @file_get_contents('http://go2rtc:1984/api/restart', false, $ctx);
    return $result !== false;
}

// The user's browser already knows the right address for this — it's
// whatever they typed to reach this very page. Default to that instead of
// making them go hunt for a Docker-internal IP (e.g. `docker exec go2rtc
// ifconfig` gives you the container's address on its private bridge
// network, like 172.18.0.2 — not reachable from a browser at all, and
// not what this field is asking for).
function detect_host_ip(): string {
    $host = $_SERVER['HTTP_HOST'] ?? $_SERVER['SERVER_NAME'] ?? '';
    $host = preg_replace('/:\d+$/', '', $host); // strip ":port" if present
    return $host !== '' ? $host : '192.168.1.1';
}

// ------------------------------------------------------------- form state --
// config.php always has *some* sane values (either the shipped example
// defaults, or whatever was last saved) — no need to special-case first-run
// vs. editing here, just read whatever's currently defined.
...
$go2rtc_host = detect_host_ip();
if (!$IS_FIRST_RUN && defined('HLS_URL') && HLS_URL && preg_match('#^https?://([^:/]+):(\d+)#', HLS_URL, $m)) {
    // Editing: show whatever was actually saved last time, not a fresh guess.
    $go2rtc_host = $m[1];
    $go2rtc_port = (int)$m[2];
}

$d = [
    'camera_ip'        => current_const('CAMERA_IP', '192.168.1.100'),
    'camera_rtsp_port' => current_const('CAMERA_RTSP_PORT', 554),
    'camera_user'      => current_const('CAMERA_USER', 'admin'),
    'camera_pass'      => $IS_FIRST_RUN ? current_const('CAMERA_PASS', '') : '', // don't echo a real secret back
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $go2rtc_host_in    = $p('go2rtc_host');
    $go2rtc_port_in    = (int)$p('go2rtc_port', '1984');
    ...
    if ($camera_pass === '') $errors[] = 'Camera RTSP password is required.';
    if ($go2rtc_host_in === '') $errors[] = 'go2rtc host/IP is required.';
    if ($rtsp_hd === '') $errors[] = 'Main RTSP stream path is required.';

    if (strpos($camera_user, ':') !== false) {
        $errors[] = "Camera username can't contain ':' — it's used as the user:pass separator for HTTP Basic Auth (cam_api.php) and a colon in the username makes that ambiguous to decode. A colon in the password is fine.";
    }
    ...
    if (empty($errors)) {
        ...
        $hls_url = sprintf('http://%s:%d/cam/index.m3u8', $go2rtc_host_in, $go2rtc_port_in);

        $tokens = [
            'HAMCAM_PASSWORD_HASH' => php_escape($password_hash),
            'CAMERA_IP'            => php_escape($camera_ip),
            'CAMERA_RTSP_PORT'     => (string)$camera_rtsp_port,
            ...
            'ONVIF_PORT'           => (string)$onvif_port,
            'MOTION_ACTIVE_START'  => php_escape($motion_start),
            'MOTION_ACTIVE_END'    => php_escape($motion_end),
            'MOTION_ALWAYS_ON'     => $always_on ? 'true' : 'false',
            'MOTION_THRESHOLD'     => (string)$motion_threshold,
            'MOTION_COOLDOWN'      => (string)$motion_cooldown,
            'SITE_TITLE'           => php_escape($site_title),
            'SITE_SUBTITLE'        => php_escape($site_subtitle),
        ];

        $config_text = render_tokens(CONFIG_TEMPLATE, $tokens);

        // go2rtc.yaml needs its own encoding for the credentials: this is a URL,
        // not a PHP string literal, and Go's URL parser is strict about it (a
        // password with a '%', '#', '?', '&', etc. would parse fine as a PHP
        // string but break go2rtc's RTSP connection — same reason snapshot.php
        // and motion_log.php urlencode() at the point of building a URL, rather
        // than relying on whatever escaping the value already went through).
        $go2rtc_tokens = $tokens;
        $go2rtc_tokens['CAMERA_USER'] = rawurlencode($camera_user);
        $go2rtc_tokens['CAMERA_PASS'] = rawurlencode($camera_pass);
        $go2rtc_text = render_tokens(GO2RTC_TEMPLATE, $go2rtc_tokens);

        $env_text = implode("\n", [
            "TZ={$tz}",
            "HAMCAM_PORT={$web_port}",
            "GO2RTC_API_PORT={$go2rtc_port_in}",
        ...
        $go2rtc_reloaded = false;
        if ($write_ok) {
            $saved = true;
            $go2rtc_reloaded = reload_go2rtc();
            if ($IS_FIRST_RUN) {
            ...
            $errors[] = 'Could not write config.php / go2rtc.yaml / .env — the web server needs '
                      . 'write permission on those files. They should already be writable '
                      . '(the container fixes this at startup); try `docker compose restart hamcam`.';
        }
    }

    if (!empty($errors)) {
        // Re-render the form with whatever was just submitted, not the stale defaults.
        $d['camera_ip'] = $camera_ip;
        $d['camera_rtsp_port'] = $camera_rtsp_port;
        $d['camera_user'] = $camera_user;
        $d['camera_pass'] = ''; // don't echo a failed password attempt back into the field
        $d['rtsp_stream_hd'] = $rtsp_hd;
        $d['rtsp_stream_sd'] = $rtsp_sd;
        $d['onvif_port'] = $onvif_port;
        $d['go2rtc_host'] = $go2rtc_host_in;
        $d['go2rtc_port'] = $go2rtc_port_in;
        $d['motion_always_on'] = $always_on;
        $d['motion_start'] = $motion_start;
        $d['motion_end'] = $motion_end;
        $d['motion_threshold'] = $motion_threshold;
        $d['motion_cooldown'] = $motion_cooldown;
...
<div class="wrap">
  ...
  <?php if ($saved): ?>
    <div class="card success">
      ...
      <?php endif; ?>
      <p class="note">Login/password, camera IP/creds, and site title take effect
      immediately.
      <?php if ($go2rtc_reloaded): ?>
        The video relay reloaded itself automatically — the new camera feed
        should be live within a few seconds.
      <?php else: ?>
        The video relay couldn't be reloaded automatically (it may still be
        starting up) — give it a minute, or run <code style="display:inline;padding:2px 6px;margin:0">docker compose restart go2rtc</code> if the feed doesn't come up.
      <?php endif; ?>
      The web UI port, timezone, and motion-detector timing/sensitivity are
      container-level settings — apply those with:</p>
  ...
  <?php else: ?>
    ...
    <form method="post" autocomplete="off">
      ...
      <div class="card">
        ...
        <div class="row">
          ...
          <div>
            ...
            <input type="password" name="camera_pass" value="<?= esc($d['camera_pass']) ?>" placeholder="<?= $IS_FIRST_RUN ? '' : '••••••••' ?>">
          </div>
        </div>
        <div class="row">
          <div>
      ...
      <div class="card">
        ...
        <p class="note">Auto-filled with the address you're using to reach
        this page right now — that's almost always correct, since go2rtc
        normally runs as part of the same docker-compose stack on the same
        machine. Only change this if go2rtc genuinely runs somewhere else.</p>
        <div class="row">
          <div>
